...
void RAM::writeByte(Address addr, ByteType val)
{
	...
	std::for_each(d_signalMap.begin(), d_signalMap.end(), [&addr, &busData](auto item) {
		if ((addr >= item.first.start) && (addr <= item.first.end)) {
			auto invokeSignal = item.second;
			(*invokeSignal)(addr, busData);
		}
	});
